...
void pmm_init(void) {
    ...
    unsigned int bitmap_addr = ((unsigned int)kernel_end + PAGE_SIZE - 1)
                               & ~(PAGE_SIZE - 1);
    // If the bitmap would land in the BIOS reserved area (0x9FC00-0xFFFFF),
    // move it to 1MB+ where RAM is guaranteed by E820 region 2.
    // This happens when the kernel BSS is large enough to push past 640KB.
    if ((bitmap_addr - 0xC0000000u) >= 0x9FC00u &&
        (bitmap_addr - 0xC0000000u) < 0x100000u) {
        // Place bitmap at 1MB physical = 0xC0100000 virtual.
        // The kernel loads at 0x10000 and the high-half mapping covers
        // 0xC0000000-0xC03FFFFF, so 0xC0100000 is always mapped.
        bitmap_addr = 0xC0100000u;
    }
...
// Allocate a page frame from below 4 MB (physical address < 0x400000).
// Used by the VMM when it needs a page table frame accessible via the
// identity map (phys_to_accessible relies on the 4MB identity mapping).
unsigned int pmm_alloc_low(void) {
    // Only scan the first 1024 frames (4MB / 4KB = 1024).
    for (unsigned int i = 0; i < 1024 && i < g_total_pages; i++) {
        if (!bit_test(i)) {
            bit_set(i);
            g_free_pages--;
            return i * PAGE_SIZE;
    ...
    return 0;  // no low frame available ? fall back to regular alloc
}
