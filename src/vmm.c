...
// Allocate a page-table frame that is accessible via phys_to_accessible().
// pmm_alloc_low() guarantees a frame below 4 MB, which is always covered
// by the identity map in PDE[0]. pmm_init() runs before vmm_init() so the
// PMM bitmap is fully initialised when this is first called.
static unsigned int alloc_pt_phys(void) {
    unsigned int phys = pmm_alloc_low();
    if (!phys) phys = pmm_alloc();
    return phys;
}
...
static int map_in_pd(unsigned int* pd, unsigned int virt,
    ...
    if (!(pde & VMM_PRESENT)) {
        unsigned int pt_phys = alloc_pt_phys();
        if (!pt_phys) return 0;
...
unsigned int vmm_resolve_pd_flags(unsigned int pd_phys, unsigned int virt) {
    unsigned int* pd  = (unsigned int*)phys_to_accessible(pd_phys);
    unsigned int  pde = pd[pd_index(virt)];
    if (!(pde & VMM_PRESENT)) return 0;
    if (pde & 0x80) return pde & 0xFFF;
    unsigned int* pt  = (unsigned int*)phys_to_accessible(pde & 0xFFFFF000);
    unsigned int  pte = pt[pt_index(virt)];
    if (!(pte & VMM_PRESENT)) return 0;
    return pte & 0xFFF;
}
