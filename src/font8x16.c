...
// IBM VGA 8x16 bitmap font, 256 glyphs x 16 scanlines, one byte per row,
// high bit = leftmost pixel. This is the native VGA BIOS font format, so
// gfx_char can index it exactly the way it indexed the plane-2 capture:
//   const unsigned char* glyph = &font8x16[ch * 16];
//   for each row: test bits with (0x80 >> col).
//
// Source: VileR's "Ultimate Oldschool PC Font Pack" VGA8.F16 — a
// pixel-perfect reproduction of the IBM VGA 8x16 ROM font, public domain
// (CC0). Embedded directly so the kernel no longer depends on capturing
// the font out of VGA plane 2 (which is unavailable once we switch to a
// linear-framebuffer video mode).

const unsigned char font8x16[FONT8X16_GLYPHS * FONT8X16_HEIGHT] = {
...
const unsigned char* font8x16_glyph(unsigned char ch) {
    return &font8x16[(unsigned int)ch * FONT8X16_HEIGHT];
}